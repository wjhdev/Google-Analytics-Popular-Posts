...
class AnalyticBridgePopularPostWidget extends WP_Widget {
  ...
  public function __construct() {
    parent::__construct(
      ...
      ['description' => __('List popular posts', 'largo')] // Args
    );
    // widget actual processes
    if (is_active_widget(false, false, $this->id_base)) {
      wp_enqueue_style(
        'abp-popular-posts-widget',
        plugins_url('css/abp-popular-posts-widget.css', __DIR__),
        'largo-stylesheet'
      );
    }
  }
  ...
  function widget($args, $instance) {
    ...
    $title = apply_filters(
      'widget_title',
      empty($instance['title']) ? __('Recent ' . $posts_term, 'largo') : $instance['title'],
      $instance,
      $this->id_base
    );
    ...
    // Start the list
    echo sprintf('<%s class="count-%d">', $olul, $instance['num_posts']);
    ...
    usort($my_query->posts, [$this, 'compare_popular_posts']);
  ...
  public function form($instance) {
    ...
    ?>
     ...
     'title'
   ); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo $instance['title']; ?>" style="width:90%;" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id(
     'num_posts'
   ); ?>"><?php _e('Number of posts to show:', 'largo'); ?></label>
			<input id="<?php echo $this->get_field_id(
     'num_posts'
   ); ?>" name="<?php echo $this->get_field_name('num_posts'); ?>" value="<?php echo $instance['num_posts']; ?>" style="width:90%;" type="number"/>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id(
     'olul'
   ); ?>"><?php _e('Display as an ordered list (with numbers) or as an unordered list?', 'largo'); ?></label>
			<select id="<?php echo $this->get_field_id(
     'olul'
   ); ?>" name="<?php echo $this->get_field_name('olul'); ?>" class="widefat">
				<option <?php selected(
      $instance['olul'],
      'ul'
    ); ?> value="ul"><?php _e('Unordered list', 'largo'); ?></option>
				<option <?php selected(
      $instance['olul'],
      'ol'
    ); ?> value="ol"><?php _e('Ordered list', 'largo'); ?></option>
			</select>
		</p>
